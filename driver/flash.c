...
void FlashWriteByte(uint32_t WriteAddress,uint8_t WriteData)
{
	...
	{                                                                                   //写入一个字节
		flash_status = FLASH_ProgramOptionByteData(START_ADDRESS + WriteAddress, WriteData);      
	}
	FLASH_LockBank1();                                                                  //上锁
}
...
void FlashWriteHarfWord(uint32_t WriteAddress,uint16_t WriteData)
{
	FLASH_UnlockBank1();                                                                //写之前解除闪存锁
